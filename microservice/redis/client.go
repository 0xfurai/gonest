...
// Client implements a Redis-style microservice client using TCP.
// Messages are routed by channel (the Pattern.Cmd field).
type Client struct {
	opts    Options
	conn    net.Conn
	scanner *bufio.Scanner
	mu      sync.Mutex
	pending map[string]chan redisResponse
	pendMu  sync.Mutex
	nextID  atomic.Int64
	done    chan struct{}
}

// NewClient creates a new Redis-style microservice client.
func NewClient(opts Options) *Client {
	return &Client{
		opts:    opts,
		pending: make(map[string]chan redisResponse),

...
// Client implements a Kafka-style microservice client using TCP.
...
type Client struct {
	...
	mu      sync.Mutex
	pending map[string]chan kafkaResponse
	pendMu  sync.Mutex
	nextID  atomic.Int64
	done    chan struct{}
}

// NewClient creates a new Kafka-style microservice client.
func NewClient(opts Options) *Client {
	return &Client{
		opts:    opts,
		pending: make(map[string]chan kafkaResponse),
...
func (c *Client) Send(ctx context.Context, pattern microservice.Pattern, data any) (json.RawMessage, error) {
	...
	respCh := make(chan kafkaResponse, 1)
	c.pendMu.Lock()
	c.pending[id] = respCh
	c.pendMu.Unlock()
...
// populateRequest fills in the request fields from either plain data or a KafkaRecord.
func (c *Client) populateRequest(req *kafkaRequest, data any) error {
	if rec, ok := data.(*KafkaRecord); ok {
